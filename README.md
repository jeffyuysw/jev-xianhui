...
打开 「先回」，首页是四张卡片，每张对应一步。**卡片徽章红色=没做，绿色=已完成**，按钮在完成后会自动隐藏；顶部有一条总览写着「还差几步」。
...
4. **自启动 + 省电无限制** —— 点「去开启」，App 会代你跳到对应的系统页面。
   > **小米 / HyperOS 必做，否则服务被冻结、读不到消息。** 这一项包含自启动和电池策略；
   > 自启动是厂商私有设置，系统不提供读取接口，所以徽章只能反映省电策略是否已设为「无限制」，
   > 自启动开关需要你自己在跳转过去的页面里确认打开。

回到桌面，悬浮窗应该已经出现；没出现就回 App 点 **「显示悬浮窗」**。

## 升级

设置页底部点 **「检查更新」**：查到新版本会直接提示并下载安装，**不必再回来下载 APK**。

首次更新时系统会要求开启「安装未知应用」权限，开完返回 App 会自动继续安装。
下载进度在系统通知栏，可以放心切出去。

> 版本检查走 GitHub Releases 的公开接口，下载走它的 CDN，所以这个项目不需要额外维护一台服务器。
> 每版都沿用同一签名，可以直接覆盖安装，API Key 等设置会保留。
...
**Q：装完没有悬浮窗 / 判断不工作？**
A：按首页四张卡逐项检查徽章颜色。红=未完成：通知使用权、悬浮窗权限、API Key 三项都要绿；第四步「自启动 + 省电无限制」影响的是后台稳定性。
